...
def get_llm(
    ...
    if url:
        logger.info("Using self-hosted LLM NIM at %s", url)
        ...
        # Special handling for nemotron-3-nano
        model_name = kwargs.get("model")
        if model_name and ("nemotron-3-nano" in model_name.lower()):
            enable_thinking = os.getenv("ENABLE_NEMOTRON_3_NANO_THINKING", "true").lower() == "true"
            llm = llm.bind(chat_template_kwargs={"enable_thinking": enable_thinking})
            logger.info("nemotron-3-nano: Setting enable_thinking=%s", enable_thinking)
        
        return llm

    logger.info("Using LLM model %s from NVIDIA API Catalog", kwargs.get("model"))
